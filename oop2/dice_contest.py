...
def main():
    ...
    while game_number < number_of_games:
        ...
        if sum(swapper.get_dice()) == sum(loaded_dice.get_dice()) == sum(mult_dice.get_dice()):
            #print("Draw!")
            pass
        elif sum(swapper.get_dice()) > sum(loaded_dice.get_dice()) and sum(swapper.get_dice()) > sum(mult_dice.get_dice()):
            #print("Dice swapper wins!")
            swapper_score += 1
        elif sum(mult_dice.get_dice()) > sum(loaded_dice.get_dice()) and sum(mult_dice.get_dice()) > sum(swapper.get_dice()):
            #print("Dice mult_dice wins!")
            mult_dice_score += 1
        else:
    ...
    print(f"Mult Dice dice won: {mult_dice_score}")



    # determine the winner
    if swapper_score == loaded_dice_score == mult_dice_score:
        print("Game was drawn")
    elif swapper_score > loaded_dice_score and swapper_score > mult_dice_score:
        print("Swapper won most games")
    elif mult_dice_score > loaded_dice_score and mult_dice_score > swapper_score:
        print("Mult Dice won most games")
    else:
